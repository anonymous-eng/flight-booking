...
export const noteSearchReducer = (state = {trip: []}, action) => {
  switch (action.type) {
    case NOTES_SEARCH_REQUEST:
      return { loading: true };
...
export const noteBookReducer = (state = {record : []}, action) => {
  switch (action.type) {
    case NOTES_BOOK_REQUEST:
      return { loading: true };
    case NOTES_BOOK_SUCCESS:
      return { loading: false, success: true, record: action.payload};
    case NOTES_BOOK_FAIL:
      return { loading: false, error: action.payload, success: false };
